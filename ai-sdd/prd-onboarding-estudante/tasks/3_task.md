...
- [ ] 3.1 Adicionar método no model `Level` (ex: `starting_level?`) que avalia se a instância atual é o `Level.order(:created_at).first` (ou `:id`).
- [ ] 3.2 Criar rota `get "/jornada", to: "dashboard#index"` e assegurar que o `root to:` aponta/redireciona para cá.
- [ ] 3.3 Construir `DashboardController` assegurando que ele herde de `Users::BaseController` para garantir que apenas alunos logados acessem.
- [ ] 3.4 Na action `index`, carregar os níveis fazendo preload de seus temas para evitar N+1: `@levels = Level.includes(:themes).order(:created_at)`.
...
- [ ] Tentar acessar `/jornada` deslogado redireciona para `/entrar`.
- [ ] Acessar `/jornada` logado carrega `@levels` e `@levels.first.starting_level?` retorna `true`.
...
### Testes Unitários
- [ ] `LevelTest`: Garantir que `starting_level?` aponta corretamente para o primeiro nível inserido, ignorando a deleção caso seja implementada (o segundo mais antigo assume a função).

### Testes de Integração
- [ ] `DashboardControllerTest`: Validar o preload da query e a autorização de sessão (bloqueio se deslogado).
